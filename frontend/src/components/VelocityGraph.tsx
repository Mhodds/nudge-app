import { useSessions, useMatchStats } from "@/hooks/useSessions";
...
const VelocityGraph = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-card-border bg-card py-10">
        ...
        <p className="px-6 text-center font-display text-xs font-semibold tracking-wider text-muted-foreground">
          MATCH VELOCITY BUILDING AFTER 2 MATCH SESSIONS
        </p>
      </div>
    );
  }
  ...
  const dataBase = matches.map((m, i) => {
    const ts = new Date(m.timestamp);
    const placeKicks = m.kicks.filter(k => k.kickType !== "try" && k.kickType !== "drop_goal");
    const acc = placeKicks.length > 0
      ? Math.round(placeKicks.filter(k => k.result === "made").length / placeKicks.length * 100)
      : 0;
    return {
      label: `M${i + 1}`,
  ...
  const allKicks = matches.flatMap(m => m.kicks).filter(k => k.kickType !== "try" && k.kickType !== "drop_goal");
  const seasonMean = allKicks.length > 0
    ? Math.round(allKicks.filter(k => k.result === "made").length / allKicks.length * 100)
    : 0;
  ...
  const CustomDot = ({ cx, cy, index }: { cx?: number; cy?: number; index?: number }) => {
    ...
    if (isLast) {
      return (
        <g>
          <circle cx={cx} cy={cy} r={8} fill="hsl(190 100% 50% / 0.25)" className="animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite]" />
    ...
    <div className="rounded-xl border border-card-border bg-card p-4">
      <ResponsiveContainer width="100%" height={180}>
        <LineChart data={data} margin={{ top: 10, right: 10, bottom: 0, left: -20 }}>
          ...
          <YAxis domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fill: "hsl(210, 15%, 65%)", fontSize: 11, fontFamily: "Orbitron" }} tickFormatter={(v: number) => `${v}%`} />
          <ReferenceLine y={seasonMean} stroke="hsl(190, 100%, 95%)" strokeDasharray="4 4" strokeOpacity={0.3} label={{ value: `SZN ${seasonMean}%`, position: "right", fill: "hsl(210, 15%, 65%)", fontSize: 11, fontFamily: "Orbitron" }} />
          <Tooltip content={<CustomTooltip />} cursor={false} />
          <Line type="monotone" dataKey="rollingAvg" stroke="url(#rollingGlow)" strokeWidth={1.5} strokeDasharray="6 3" dot={false} activeDot={false} />
          <Line type="monotone" dataKey="accuracy" stroke="url(#velocityGlow)" strokeWidth={2.5} dot={<CustomDot />} activeDot={{ r: 5, fill: "hsl(190, 100%, 50%)" }} />
        </LineChart>
      </ResponsiveContainer>
      <FormChip />
      <div className="mt-2 flex items-center justify-center gap-5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-display text-[10px] tracking-wider text-muted-foreground">MATCH ACC</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-4 rounded-full border-t border-dashed" style={{ borderColor: "hsl(45, 100%, 60%)" }} />
          <span className="font-display text-[10px] tracking-wider text-muted-foreground">ROLLING 3</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-4 rounded-full border-t border-dashed border-muted-foreground/50" />
          <span className="font-display text-[10px] tracking-wider text-muted-foreground">SZN AVG</span>
        </div>
      </div>
    </div>
  );
